...
public class PromptManager : MonoBehaviour
{
    ...
    public IEnumerator MoveAway()
    {
        ...
        while (Time.time - offset <= timeToPlay)
        {
            ...
            mainTrans.position = new Vector3(Screen.width/2f, Screen.height/2f + (directionUp ? -1 : 1)*Screen.height*position.Evaluate((Time.time - offset)/timeToPlay),0);
            yield return null;
        }
    }
    ...
    public IEnumerator MoveTowards()
    {
        ...
        while (offset- Time.time>=0)
        {
            ...
            mainTrans.position = new Vector3(Screen.width / 2f, Screen.height / 2f + (directionUp ? -1 : 1) * Screen.height * position.Evaluate((offset - Time.time) / timeToPlay), 0);
            yield return null;
        }
    }
    ...
    void Update()
    {
        if (bad)
        {
            ...
            {
                input.transform.localPosition = pos + new Vector3(Mathf.Sin(Mathf.PingPong(12*(Time.time - badTime), 1f))*5f, 0, 0);
            }
        }
    }
}
